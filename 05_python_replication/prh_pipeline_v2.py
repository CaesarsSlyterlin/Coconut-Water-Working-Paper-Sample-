...
def fetch_municipality_boundaries(year=None, scale="4500k"):
    """Fetch municipality boundaries from Statistics Finland's WFS.

    Statistics Finland serves administrative boundaries through an
    open WFS at geo.stat.fi (no API key). By default this requests the
    layer alias without a year suffix (tilastointialueet:kunta4500k),
    which points to the latest municipal division -- the same usage
    documented in standard Python GIS teaching material (e.g. the
    AutoGIS course and the Introduction to Python for Geographic Data
    Analysis book, both CC BY 4.0). Pass `year` (e.g. 2023) to pin a
    specific historical division instead; the yearly layers
    (tilastointialueet:kunta4500k_2013 onwards) remain available.
    Scale 4500k (1:4,500,000) is the generalised version, small and
    well suited to a whole-country choropleth. Data arrive in
    EPSG:3067 (ETRS89 / TM35FIN).
    """
    import geopandas as gpd

    layer = f"tilastointialueet:kunta{scale}"
    if year is not None:
        layer += f"_{year}"
    wfs_url = (
    ...
    print(f"  fetching municipality boundaries ({layer}) from geo.stat.fi")
    return gpd.read_file(wfs_url)


def plot_municipality_map(firms, geojson_path=None,
                          out_png="firms_by_municipality.png"):
    """Plot firm counts by municipality on a Finland map.

    If `geojson_path` is given, boundaries are read from that local
    file; otherwise they are fetched directly from Statistics
    Finland's WFS via fetch_municipality_boundaries(). The join key is
    the 3-digit municipality code (zero-padded), matching
    municipality_code parsed in Step 2.
    """
    try:
        import geopandas as gpd
    ...
    if geojson_path is not None:
        ...
        geo = fetch_municipality_boundaries()
    counts = (firms.groupby("municipality_code")
                   .size()
                   .rename("n_firms")
                   .reset_index())
    counts["municipality_code"] = counts["municipality_code"].str.zfill(3)
    # The municipality-code column is 'kunta' in Statistics Finland's
    # WFS layers; probe defensively in case a local file names it
    # differently.
    code_col = next((c for c in ("kunta", "KUNTA", "kuntakoodi")
                     if c in geo.columns), None)
    if code_col is None:
        print(f"  no municipality-code column found "
              f"(columns: {list(geo.columns)[:8]}...); skipping map")
        return
    geo[code_col] = geo[code_col].astype(str).str.zfill(3)
    merged = geo.merge(counts, left_on=code_col,
...
def main():
    ...
    print("  skipped by default; run plot_municipality_map(firms) to "
          "fetch boundaries from Statistics Finland's WFS and draw "
          "the map, or pass geojson_path= to use a local file")
